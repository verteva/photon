<template>
  <div :id="`ph-slider-${id}`" class="ph-w-full ph-relative">
    ...
    />
    <div :class="trackWrapperClassList">
      <div :class="trackClassList" class="ph-bg-grey5 ph-w-full" />
      <div
...
import { SliderData, SliderTrackRef, StepDataType } from './types';
...
export default Vue.extend({
  ...
  computed: {
    ...
    barScale(): number {
      if (this.steps)
        return (
          (this.innerValue - this.stepData.min) /
          (this.stepData.max - this.stepData.min)
        );
    ...
    trackClassList(): string[] {
      return ['ph-h-full', 'ph-absolute', 'ph-top-0', 'ph-left-0'];
    },
  ...
  methods: {
    ...
    onDrag() {
      ...
      if (this.steps) {
        ...
        this.$emit(
          'input',
          stepIndex * this.stepData.increment + this.stepData.min
        );
      } else {
